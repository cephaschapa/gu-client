        ...
        <div className="flex justify-between px-3 items-center h-32 w-full bg-gray-100 rounded-2xl">
            ...
            { //data.current.weather[0].icon
                    current ==  curr? <>
                        ...
                        <div className="flex flex-col space-y-3 items-end">
                            <div className="text-2xl text-gray-600"><span className="font-bold text-blue-600">{ Math.ceil(data?.daily[current].temp.min-273.15)}</span><sup>o</sup>C <span className="font-bold text-yellow-600">{ Math.ceil(data?.daily[current].temp.max-273.15)}</span><sup>o</sup>C </div>
                            <div className="flex flex-col justify-end items-end space-y-1">
                                <div className="flex space-x-2"><LocationMarkerIcon className="h-6 w-6"/><span className="font-bold pr-2">{location.region},</span> {location.country}</div>
                                <div className="flex space-x-2 text-xs  items-center text-gray-400"><ClockIcon className="h-3 w-3"/><span className="font-bold pr-2">Updated -</span>{moment.unix(data?.current.dt).fromNow()}</div>
                            </div>
